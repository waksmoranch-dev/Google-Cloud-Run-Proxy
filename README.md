...
2.  **Set environment variables:**
    ```bash
    export PORT=8080
    export V2RAY_SERVER_IP=8.212.160.122  # Replace with your target IP
    ```
...
docker run -p 8080:8080 \
  -e PORT=8080 \
  -e V2RAY_SERVER_IP=8.212.160.122 \
...
2.  **Run the container:**
    ```bash
    docker run -p 8080:8080 -e PORT=8080 -e V2RAY_SERVER_IP=8.212.160.122 cloud-run-proxy
    ```
...
gcloud run deploy cloud-run-proxy \
  ...
  --set-env-vars V2RAY_SERVER_IP=8.212.160.122
```
